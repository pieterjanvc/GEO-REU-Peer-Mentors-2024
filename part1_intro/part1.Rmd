...
Each section will first introduce a new topic together with some code examples. Some examples have simple instructions below for you to explore them in a bit more detail (called: EXPLORE A BIT MORE). At the end of each section will be a `Try it yourself` exercise that combines all you have learned and requires a bit more thinking. 
...
```
 _EXPLORE A BIT MORE: Try changing the text_
...
_EXPLORE A BIT MORE: Try to replace the mean function with some other basic functions you can execute on vectors of numbers like `min` or `max` or `sum`_
...
3. Now make sure that the sorting happens in _decreasing_ order (i.e. highest to lowest). You will need to set an argument of the sort function to make this happen (look at the documentation of the sort function)
