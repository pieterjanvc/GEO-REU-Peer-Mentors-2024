...
#### Use the `toupper()` function to change all state names to be written in capital letters (update the _states_ column)
...
*Get the mean and standard deviation of the affected acres by state. Note that NA means there is a missing value. In this case that is because there only is one row for those states and you need at least two values to calculate standard deviation*
