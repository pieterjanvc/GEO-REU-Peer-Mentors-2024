...
#### Continue from the last iris flowers scatter plot and colour all the points blue
...
*ggplot will automatically generate colours for different species names*
...
*`stat = "identity"` should always be set when you are creating bar charts where you summarised the data per bar already*
...
sumData <- flowers |> 
  ...
  ungroup()
```
_This is a good refresher of Part 2 on working with data frames. Should you really get stuck scroll down to the very end to see the code_
...
Let's look at a wine tasting experiment. Experts rated red wines on a ten point scale. In addition specific wine properties were measured. Let's evaluate some of the result visually.
...
```
_Again if you get stuck scroll down to the very end to see the code_
...
#### Save the bar chart as a PNG image `wine_sulfate_quality.png` with dimensions 550px x 1150px (width x height)
...
*Set colours across a whole range of numbers (number of colours provided here are equally spaced between min and max)*
...
Summarise the winedata so that you get the average `mean()` sulphates concentration per quality level and save this to a new varliable `sulQual`
