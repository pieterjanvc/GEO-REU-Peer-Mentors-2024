...
We'll put the measured pH level on the x-axis and the amount of dissolved oxygen on the y-axis
...
Depending on the data we have and what we want to visualise, we can either specify plot style attributes for the whole plot or have them be driven by the data.
...
waterPlot
```
_Note that the size of the point is now defined by the value of n_actions and a legend appears_
...
#### Now change this so that each species has a different colour
```{r}
...
### Add axis labels and title - `labs()`
```{r}
waterPlot <- ggplot(water, aes(x = pH, y = Oxygen, size = Temp)) +
  geom_point() +
  labs(x = "Water pH", 
...
When creating bar charts with ggplot you have to summarise the data first if 
needed so it contains the position and height of each bar.
...
ggsave(filename = "oxygenation.png", plot = flowersPlot, width = 1800, height = 900, units = "px")
```

## Bringing it all together

Let's look at a wine tasting experiment. Experts rated red wines on a ten point scale. In addition specific wine properties were measured. Let's evaluat some of the result visually.
...
When setting `colour` in the `aes()` function, ggplot will use a default set of
colours. You can change this by providing your own colour scale
...
flowersPlot
```
_Colours when there are a discrete set of possibilities_
...
flowersPlot
```
_Set colours across a whole range of numbers (number of colours provided here are equally spaced between min and max)_

## Add text labels

You can add text labels instead of points with `geom_text`
```{r}
flowersPlot <- ggplot(flowers, aes(x = Petal.Length, y = Petal.Width, 
                             label = Species)) + 
  theme_minimal() +
...
flowersPlot
```
_You can set `check_overlap = TRUE` to clean up overlapping text. You can also change to `geom_label()` if you want the labels to have a background colour_
...
flowersPlot
```
_The `method` here is set to a linear model "lm", and standard deviation `se` is hidden_
